...
public interface InteractionLogRepository extends JpaRepository<InteractionLog, Long> {
	...
	@Query(value = 
			"SELECT * FROM interaction_log where document_id = :id order by saved_time desc limit 1;",
			nativeQuery = true)
	Optional<UpdateLog> getMostRecentByDocumentId(@Param("id") Long id);

	@Query(value = 
			"SELECT * FROM interaction_log where log_time >= :start and log_time <= :end",
			nativeQuery = true)
	...
	@Query(value = 
			"SELECT * FROM interaction_log where user_id = :userid and log_time >= :date_start and log_time <= :date_end",
			nativeQuery = true)
