...
//	boolean existsByEisdocAndFilename(EISDoc eisdoc, String filename);
	...
	@Query(value = "SELECT n1.* FROM test.nepafile n1 JOIN test.nepafile n2 ON n1.document_id = n2.document_id " + 
			"WHERE n1.folder != n2.folder;", nativeQuery = true)
	List<NEPAFile> getMultiFolder();


	@Query(value = "SELECT COUNT(DISTINCT (n1.folder)) FROM test.nepafile n1 WHERE n1.document_id = :id", 
			nativeQuery = true)
	Long countDistinctFoldersByDocumentId(@Param("id") Long id);

	@Query(value = "SELECT DISTINCT (n1.folder) FROM test.nepafile n1 WHERE n1.document_id = :id", 
			nativeQuery = true)
